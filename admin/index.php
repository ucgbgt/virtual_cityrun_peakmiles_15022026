    ...
    try {
        $stmt = $db->prepare("
            SELECT u.name, r.category, r.target_km, r.status,
                ...
                COALESCE(SUM(TIME_TO_SEC(rs.run_time)), 0) AS total_time_sec,
                SEC_TO_TIME(COALESCE(SUM(TIME_TO_SEC(rs.run_time)), 0)) AS total_time
            FROM registrations r
            ...
            ORDER BY total_km DESC, total_time_sec ASC
            LIMIT 5
        ");
        $stmt->execute([$event['id']]);
      ...
      <div class="analytics-grid">
        ...
        <div class="db-card" style="display:flex;flex-direction:column;">
          ...
          <?php foreach ($leaderboard as $i => $p):
            ...
            $timeDisplay = ($p['total_time_sec'] > 0) ? substr($p['total_time'], 0, 5) : '—';
          ?>
          <div class="lb-row">
            ...
            <div style="text-align:right;flex-shrink:0;">
              ...
              <div style="font-size:10px;color:var(--gray);margin-top:1px;">
                <i class="fa fa-clock" style="font-size:9px;margin-right:2px;"></i><?= $timeDisplay ?>
              </div>
            </div>
          </div>
          <?php endforeach; ?>
